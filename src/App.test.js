...
describe("the GuitaristInfo component is rendered with multiple values.", () => {
  ...
  it("renders an error message when a faulty name is provided", async () => {
    ...
    expect(
      screen.getByText(
        "Sorry, we could not find any results for that guitarist..."
      )
    ).toBeInTheDocument();
  });
});
...
describe("Metronome Component", () => {
  ...
  it("gives the metronome a pulsing effect as it plays", async () => {
    ...
    await waitFor(() => {
      ...
      expect(displayCircle).toHaveStyle(
        "animation: pulse-animation 600ms infinite;"
      );
    });
  ...
  it("changes the speed and value of the metronome", async () => {
    ...
    await waitFor(() => {
      expect(displayCircle).not.toHaveStyle(
        "animation: pulse-animation 600ms infinite;"
      );
    });
...
describe("Tuner component", () => {
  it("renders the Tuner component", () => {
    const { getByText, getAllByTestId } = render(<Tuner />);
...
describe("Popup component", () => {
  it("renders the Popup component", () => {
    ...
    const popupSpy = jest
      .spyOn(React, "useRef")
      .mockReturnValueOnce({ current: { style: null } });
    /* eslint-enable no-unused-vars */
    render(
      <Popup order={1} isShowing={true} updateOrder={jest.fn()} message="Hi" />
    );
